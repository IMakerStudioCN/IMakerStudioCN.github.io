...
资源链接可直接粘贴含中文路径或中文查询参数的 HTTP/HTTPS 链接。同步脚本会在写入 resources.json 前清理零宽字符和特殊空格，再转换为标准百分号编码。完整网盘分享文本中的说明文字会被过滤，只同步其中的链接；提取码或访问密码建议写入资源简介。无法解析的链接会被跳过，Action 日志会显示飞书记录 ID。
