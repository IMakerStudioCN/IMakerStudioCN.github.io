...
`资源链接` 可直接粘贴含中文路径或中文查询参数的 HTTP/HTTPS 链接；同步脚本会在写入 `resources.json` 前自动转换为标准百分号编码。复制网盘分享文本时，脚本会提取其中的链接；提取码或访问密码建议单独写入“资源简介”。无法解析的链接会被跳过，并在 Action 日志中显示飞书记录 ID。
